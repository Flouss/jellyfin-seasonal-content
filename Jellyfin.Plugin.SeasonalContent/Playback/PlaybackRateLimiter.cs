...
/// <summary>
/// Suppresses a repeat request for the same (user, TMDb id) within a short window, per
...
/// actually prevents duplicate requests long-term).
/// </summary>
public sealed class PlaybackRateLimiter
{
    private readonly IClock _clock;
    private readonly TimeSpan _window;
    private readonly ConcurrentDictionary<(Guid UserId, int TmdbId), DateTimeOffset> _lastAllowedAt = new();
    ...
    /// <summary>
    /// Checks whether a request for this (user, TMDb id) pair should proceed right now, and - if
    /// so - starts a new suppression window for it.
    /// </summary>
    ...
    public bool ShouldAllow(Guid userId, int tmdbId)
    {
        var now = _clock.UtcNow;
        var key = (userId, tmdbId);
